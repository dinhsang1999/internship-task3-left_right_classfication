...
class CamExtractor():
    ...
    def forward_pass_on_convolutions(self, x):
        ...
        """
        conv_output = None
        for module_name, module in self.model.module.resnet._modules.items():
            print(module_name)
            if module_name == 'fc':
    ...
    def forward_pass(self, x):
        ...
        x = self.model.module.resnet.fc(x)
        return conv_output, x
...
class GradCam():
    ...
    def generate_cam(self, input_image, target_index=None):
        ...
        # Zero grads
        self.model.module.resnet.fc.zero_grad()
        ...
        for i, w in enumerate(weights):
            cam += w * target[i, :, :]
        cam = cv2.resize(cam, (int(224*1.5), 224))
...
def save_class_activation_on_image(org_img, activation_map, file_name):
    ...
    # Heatmap on picture
    org_img = cv2.resize(org_img, (int(224*1.5), 224))
...
def preprocess_image(cv2im, resize_im=True):
    ...
    # Resize image
    if resize_im:
        cv2im = cv2.resize(cv2im, ( int(224*1.5), 224))
